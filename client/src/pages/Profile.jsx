          ...
          {userRole !== "manager" && (
            <>
              ...
              <section>
                ...
                {bookings.length === 0 ? (
                  <div className="bg-white rounded-xl border border-dark-chocolate/10 p-10 text-center text-dark-chocolate/60">
                    No bookings yet. Book a tour from Explore to see tickets
                    here.
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 justify-items-center xl:justify-items-start">
                    {bookings.slice(0, 6).map((booking) => (
                      <Link
                        key={booking.booking_id}
                        to={`/tour/${booking.museum?.id}/payment-success/${booking.booking_id}`}
                        state={{ booking }}
                        className="hover-3d cursor-pointer w-full"
                      >
                        <div className="card w-96 bg-black text-white bg-[radial-gradient(circle_at_bottom_left,#ffffff04_35%,transparent_36%),radial-gradient(circle_at_top_right,#ffffff04_35%,transparent_36%)] bg-size-[4.95em_4.95em] shadow-xl">
                          <div className="card-body pt-5 pb-4">
                            <div className="flex justify-between mb-7">
                              <div className="font-bold">KYUBIPAY</div>
                              <div className="text-5xl opacity-10">❁</div>
                            </div>
                            <div className="text-lg mb-2 opacity-40 font-mono tracking-wide break-all leading-snug">
                              {booking.ticket_code}
                            </div>
                            <div className="text-sm mb-2 opacity-60 truncate">
                            ...
                            <div className="flex justify-between -mt-1">
                              <div>
                                ...
                                <div>{formatDate(booking.tour_date)}</div>
                              </div>
                              <div className="text-right">
                                <div className="text-xs opacity-20">TIME</div>
                                <div>
                                  {toTwelve(booking.start_time)} -{" "}
                                  {toTwelve(booking.end_time)}
                                </div>
                              </div>
                            </div>
                          </div>
